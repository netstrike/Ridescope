...
- `build` app allineato al `versionCode` Android usato da Play Console
- `version_name` app in formato semver
...
- la schermata di creazione release accetta l'AAB; se Play Console segnala codice nativo senza simboli, lo ZIP `native-debug-symbols-v<versionName>-code<versionCode>.zip` generato dallo script e opzionale e si carica, quando disponibile, dall'App Bundle Explorer per la stessa versione
- se Play Console non espone l'upload dei simboli per il bundle, l'avviso sui simboli nativi e informativo e non blocca il rilascio
...
- la pagina `Aggiornamento` espone solo il controllo firmware, lo stato aggiornamento, il progresso OTA e le azioni `CONTROLLA ORA` / `UPD`; gli aggiornamenti app sono gestiti da Google Play
...
- l'app non controlla piu aggiornamenti applicativi: versione e distribuzione dell'app sono gestite da Google Play tramite `versionCode`
